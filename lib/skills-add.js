...
export function resolveSkillSource(source) {
  ...
  const clone = spawnSync('git', ['clone', '--depth', '1', '--quiet', source, scratch], { encoding: 'utf8' })
  if (clone.status !== 0) {
  ...
  rmSync(scratch, { recursive: true, force: true })
  throw new Error(`skills add: no skills found in ${source} (expected a skills/ directory or SKILL.md files)`)
}
