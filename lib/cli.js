...
function parseArgs(argv) {
  const flags = argv.filter(a => a.startsWith('-') && !a.startsWith('--root') && !a.startsWith('--ci'))
  const positionals = argv.filter(a => !a.startsWith('-'))
  // `--version`/`--help` are commands spelled with dashes, not flags: they
  // answer alone and must win over the positional command fallback.
  const dashCommand = argv.includes('--version') || argv.includes('-v')
    ? '--version'
    : argv.includes('--help') || argv.includes('-h')
      ? '--help'
      : undefined
  const command = dashCommand ?? positionals[0] ?? '--help'
