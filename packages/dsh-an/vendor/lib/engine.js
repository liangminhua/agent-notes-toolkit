...
export function engineCiSetup(rootFlag, options = {}) {
  ...
  if (vendor === 'gitlab') {
    const target = join(root, '.gitlab', 'agent-notes.yml')
    if (options.confirm || !existsSync(target)) {
      mkdirSync(dirname(target), { recursive: true })
      writeFileSync(target, gitlabWorkflow())
      return { ok: true, wrote: target, lines: [`ci-setup: wrote ${target} (append \`include: '.gitlab/agent-notes.yml'\` to .gitlab-ci.yml)`] }
    }
    return { ok: false, lines: [`ci-setup: ${target} already exists — refusing to overwrite. Review it, or re-run with --confirm.`] }
  }
  return {
    ok: true,
...
/** The standalone GitLab include file content. */
export function gitlabWorkflow() {
  return [
    '# Agent Notes verification for GitLab CI. Include it from .gitlab-ci.yml:',
    "#   include: '.gitlab/agent-notes.yml'",
    'agent-notes:',
    '  script:',
    '    - npm install',
    '    - npm exec an verify',
